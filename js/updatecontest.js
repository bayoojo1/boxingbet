...
$(document).ready(function() {
    $("input[name='win']").click(function() {
        win = $("input[name='win']:checked").val()
        odd = win.split('-')[0]
        contestId = win.split('-')[1]
        $.ajax({
          url: 'procedures/updateWin.php',
          type: 'POST',
          dataType: 'text',
          data: JSON.stringify({ odd: odd, contestId: contestId }),
          success: function(response) {
            if(response.indexOf('success') !== -1) {
                swal({
                  title: "Success",
                  text: "Winning odd successfully set",
                  icon: "success",
                });
            }
          }
